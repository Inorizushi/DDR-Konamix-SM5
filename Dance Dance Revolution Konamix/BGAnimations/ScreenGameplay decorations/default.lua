...
for pn in ivalues(GAMESTATE:GetEnabledPlayers()) do
	t[#t+1] = Def.ActorFrame{
		LoadActor("LifeMeterBar under")..{
			...
			OnCommand=function(self) if pn == PLAYER_1 then self:x(SCREEN_CENTER_X-180):zoomx(1) else self:x(SCREEN_CENTER_X+180):zoomx(-1) end end;
...
for pn in ivalues(GAMESTATE:GetEnabledPlayers()) do
	for i = 1,17 do
		t[#t+1] = Def.Sprite{
			...
			InitCommand=function(self) if pn == PLAYER_1 then self:x(SCREEN_CENTER_X-280+12*i) else self:x(SCREEN_CENTER_X+280-12*i) end end;
