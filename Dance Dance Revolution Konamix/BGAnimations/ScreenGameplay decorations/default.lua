	...
	t[#t+1] = Def.ActorFrame{
		LoadActor("LifeMeterBar under")..{
			...
			OnCommand=function(self) 
				if pn == PLAYER_1 then self:x(SCREEN_CENTER_X-180):zoomx(1) 
								if IsUsingWideScreen() then
									self:x(SCREEN_LEFT+162)
								end;
				elseif pn == PLAYER_2 then self:x(SCREEN_CENTER_X+180):zoomx(-1)
							if IsUsingWideScreen() then
								self:x(SCREEN_RIGHT-162)
							end;
						end
				end;
	...
	for i = 1,17 do
		t[#t+1] = Def.Sprite{
			...
			InitCommand=function(self) 
				if pn == PLAYER_1 then self:x(SCREEN_CENTER_X-280+12*i)
			
												if IsUsingWideScreen() then
									self:x(SCREEN_LEFT+59+12*i)
								end;
								
					elseif pn == PLAYER_2 then self:x(SCREEN_CENTER_X+280-12*i)
						
												if IsUsingWideScreen() then
								self:x(SCREEN_RIGHT-59-12*i)
							end;
						end
				end;
			OnCommand=function(self)
