...
for i=1,Params.NumParticles do
	...
	Name="Particle"..i;
		...
		self:y(math.random(SCREEN_TOP/2+(self:GetHeight()/2),SCREEN_BOTTOM/2-(self:GetHeight()/2)));
		--self:z(math.random(-64,0));
	end;
		OnCommand=cmd(zoom,2.5;diffuse,ColorLightTone(color("#ffd400")));
	};
end
